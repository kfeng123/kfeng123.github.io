---
layout: post
title: "bernstein-von mises theorem证明报告d"
...
<b>$$\int [\sqrt {p_{\theta+h}}-\sqrt {p_{\theta}}-\frac{1}{2}h^T\dot{l_\theta}\sqrt{p_\theta}]^2\, d\mu=o(||h||^2)$$</b>
...
<b>$$\log \prod_{i=1}^n\frac{p_{\theta+h_n/\sqrt n}}{p_\theta}(X_i)=\frac{1}{\sqrt n}\sum_{i=1}^n h^T\dot{l}_\theta(X_i)-\frac{1}{2}h^TI_\theta h+o_{P_\theta}(1)$$</b>
...
<b>$$P_{\theta_0}^n \phi_n \to 0,\sup_{||\theta-\theta_0||\ge \epsilon}P^n_\theta (1-\phi_n)\to 0$$</b>
...
<b>$$\pi_n^C=\frac{\pi_n(h)1_{\{C\}}(h)}{\int_C \pi_n(h)\,dh}$$</b>
...
所以有：
<b>$$||P_{\vec{H}_n|\vec{X}_n}-P^{C_n}_{\vec{H}_n|\vec{X}_n} ||\leq 2P_{\vec{H}_n|\vec{X}_n}(C^c_n)$$</b>

下面证明上式右边在$P_{n,U}$下$L^1$收敛到0，其中U是一个以0为圆心，半径固定的球。首先由条件3以及$P_{n,U}\triangleleft P_{n,0}$，我们有
<b>$$P_{n,U}P_{\vec{H}_n|\vec{X}_n}(C^c_n)=P_{n,U}[P_{\vec{H}_n|\vec{X}_n}(C^c_n)(1-\phi_n)]+o(1)$$</b>
可以对上式右边第一项进行如下操作：
<b>$$
\begin{align}
